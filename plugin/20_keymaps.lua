...
local todo_picker = '<Cmd>lua MiniPick.builtin.grep({ pattern = "TODO"}, { source = { name = "TODOs" }})<CR>'

-- ChatGPR generated function to filter the current buffer with TODO finders
local pick = require("mini.pick")
local function todo_picker_cur_file()
  ...
  pick.start({
    source = {
      name = "TODOs (current buffer)",
      items = items,

      choose = function(item)
        if not item then return end

        -- Defer jump until picker is fully closed
        vim.schedule(function()
          if not vim.api.nvim_buf_is_valid(item.buf) then return end
      ...
      preview = function(bufnr, item)
        ...
        local context = 10
        local start = math.max(1, item.lnum - context)
        local finish = math.min(
          vim.api.nvim_buf_line_count(item.buf),
          item.lnum + context
        )

        local preview_lines =
          vim.api.nvim_buf_get_lines(item.buf, start - 1, finish, false)

        vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, preview_lines)

        -- Highlight the TODO line
        local ns = vim.api.nvim_create_namespace("MiniPickTodoPreview")
        vim.api.nvim_buf_clear_namespace(bufnr, ns, 0, -1)

        local hl_line = item.lnum - start
...
-- -- This would work, if mini.extra would allow the API to return the source of a picker, not just start the picker
-- local pick = require("mini.pick")
-- local extra = require("mini.extra")
-- local function todo_picker_cur_file()
--   local buf = vim.api.nvim_get_current_buf()
--   local lines = vim.api.nvim_buf_get_lines(buf, 0, -1, false)
--   -- Build buf_lines-style items
--   local items = {}
...
--     end
--   end
--   -- Generate the buf_lines source (this includes preview!)
--   local source = extra.gen_source.buf_lines({
--     scope = "current",
--   })
--   -- Override only what we need
--   source.items = items
--   source.name = "TODOs (current buffer)"
